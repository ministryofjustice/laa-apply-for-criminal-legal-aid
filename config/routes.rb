def edit_step(name, opts = {}, &block)
  ...
  return unless name.to_s.include?('_') && !block
...
def crud_step(name, opts = {})
  edit_step name, only: [] do
    resources only: [:edit, :update, :destroy],
      ...
      # temp underscored routes
      if parent_resource.actions.include?(:destroy) && !name.to_s.include?('_')
        get 'confirm_destroy', to: "#{opts.fetch(:alias, name)}#confirm_destroy",
on: :member
      end
    end
  end
  # temp underscored routes
  return unless name.to_s.include?('_')
  ...
  unless opts.fetch(
    ...
    get "#{name}/:#{opts.fetch(:param)}/confirm_destroy",
        to: "#{opts.fetch(:alias, name)}#confirm_destroy"
  end
end
