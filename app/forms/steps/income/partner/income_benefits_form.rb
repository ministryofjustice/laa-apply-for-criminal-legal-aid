  ...
  module Income
    module Partner
      class IncomeBenefitsForm < Steps::BaseFormObject
        ...
        IncomeBenefitType.values.each do |type| # rubocop:disable Style/HashEachMethods
          ...
          define_method :"#{type}=" do |attrs|
            ...
            record = IncomeBenefitFieldsetForm.build(
              IncomeBenefit.new(payment_type: type.to_s, ownership_type: OwnershipType::PARTNER.to_s, **attrs),
              crime_application:
            )
        ...
        def persist!
          crime_application.income.update(partner_has_no_income_benefits:)
        end
      end
    end
  end
