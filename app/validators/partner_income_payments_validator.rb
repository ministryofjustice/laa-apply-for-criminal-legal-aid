class PartnerIncomePaymentsValidator < ActiveModel::Validator
  attr_reader :record
  ...
  def validate(record)
    ...
    record.errors.add(:base, :none_selected) if record.partner_has_no_income_payments.blank?
  end
  ...
  # `activemodel.errors.models.steps/income/partner/income_payment_fieldset_form.summary.x.y`
  def error_message(obj, error)
    payment_type = I18n.t(
      obj.payment_type,
      scope: [:helpers, :label, :steps_income_partner_income_payments_form, :types_options]
    )&.downcase!
